...
function predict(X)
    ...
    Y = {}
    for j=1,X:size(1) do
        local cur_word = torch.Tensor(1)
        cur_word[1] = X[j]
        cur_word = cur_word:cuda()
    ...
    return Y
    -- io.write(idx_to_word[cur_word] + " " + idx_to_punc[predicted_punc] + "\n")
end
...
local DATA_DIR = '../data/'
local SEQ_LEN = 50
local loader = MinibatchLoader.create(DATA_DIR .. 'corpus.txt', BATCH_SIZE, SEQ_LEN, {.8,.1,.1})
...
repeat
    ...
    y = predict(x)
    -- print("YYYYY", y[1]:size())
    io.write(loader:ints_to_text(x, y) .. '\n\n')
    io.flush()
until input=="exit"
