...
require 'util.OneHot'
require 'util.utils'
-- local CharSplitLMMinibatchLoader = require 'MinibatchLoader'
...
-- cmd:option('-corpus_file','data/corpus.txt','corpus')
...
if opt.gpuid >= 0 and opt.opencl == 1 then
    ...
    if ok and ok2 then
        ...
        print(cltorch.setDevice)
        print("device count", cltorch.getDeviceCount())
        print(opt.gpuid + 1)
...
local loader = MinibatchLoader.create(opt.data_dir .. 'corpus.txt', opt.batch_size, opt.seq_length, split_sizes)
...
if string.len(opt.init_from) > 0 then
    ...
    protos = {}
    -- protos.embed = nn.LookupTable(vocab_size+1, 300)
    if opt.model == 'lstm' then
        protos.rnn = LSTM.lstm(vocab_size+1, #loader.idx_to_punc, opt.rnn_size, opt.num_layers, opt.dropout)
    elseif opt.model == 'gru' then
        protos.rnn = GRU.gru(vocab_size+1, opt.rnn_size, opt.num_layers, opt.dropout)
    elseif opt.model == 'rnn' then
        protos.rnn = RNN.rnn(vocab_size+1, opt.rnn_size, opt.num_layers, opt.dropout)
    end
...
-- params, grad_params = model_utils.combine_all_parameters(protos.embed, protos.rnn)
...
function prepro(x,y)
    ...
    x = x:transpose(1,2):contiguous() -- swap the axes for faster indexing. commented because WTF
...
function eval_split(split_index, max_batches)
    ...
    for i = 1,n do -- iterate over batches in the split
        ...
        x,y = prepro(x,y)
        -- forward pass
        local seq_len = math.min(x:size(1), opt.seq_length) -- TODO: DO SOMMETHING BETTER
        for t=1,seq_len do
    ...
    loss = loss / opt.seq_length / n -- TODO: ESPECIALLY HERE
    return loss
end
...
function feval(x) -- what's going on with feval http://rnduja.github.io/2015/10/26/deep_learning_with_torch_step_7_optim/
    ...
    x,y = prepro(x,y)
    ------------------- forward pass -------------------
    -- print("init_state_GLOBAL", init_state_global)
    local rnn_state = {[0] = init_state_global}
    local predictions = {}           -- softmax outputs
    local loss = 0
    local seq_len = math.min(opt.seq_length, x:size(1))
    for t=1,seq_len do
        clones.rnn[t]:training() -- make sure we are in correct mode (this is cheap, sets flag)
        -- x should have shape time_steps x batch_size. so in my case N x 1
        -- print("XSHAPE", x[t]:size())
        -- print("RNNSHAPE", rnn_state[t-1]) -- this should be batch_size x rnn_size. So 1 x 128 in my case
        local lst = clones.rnn[t]:forward{x[t], unpack(rnn_state[t-1])}
        rnn_state[t] = {}
    ...
    loss = loss / opt.seq_length --TODO FIX
    ------------------ backward pass -------------------
